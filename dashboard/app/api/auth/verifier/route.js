...
export async function GET(req) {
  ...
  const h = req.headers;
  // The assistant, search, AI option and message wall APIs are called by scripts and fetch(): a JSON 401 rather than the login page
  if (/^\/api\/(assistant|recherche|ia|messages)(\/|\?|$)/.test(h.get('x-forwarded-uri') || '')) {
